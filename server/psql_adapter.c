...
static MString *psql_prepare_conninfo(const char *database, const char *host, const char *port, const char *user, const char *pass, const char *extra_conninfo)
{
  ...
  portnum = resolve_service(port, DEFAULT_PG_PORT);
...
{
  if(snprintf(uri, size, "postgresql://%s@%s:%d/%s", pg_user, pg_host, resolve_service(pg_port, DEFAULT_PG_PORT), db->name) >= size)
    return 1;
  return 0;
}
