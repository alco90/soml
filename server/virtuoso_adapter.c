  ...
  for (c=0; c<max; c++) {
    ...
    if (osd) {
      ...
      n += mstring_sprintf(insert,"\t\t`iri(bif:concat(\"http://%s:%s/%s/\",bif:sequence_next('<http://%s:%s/%s>')))` %s `iri(bif:concat(\"http://%s:%s/%s/\",bif:sequence_next('<http://%s:%s/%s>')))` .\n",
        vir_host,vir_port,table->schema->name,vir_host,vir_port,db->name,osd->verb, vir_host,vir_port,field->name,vir_host,vir_port,db->name);
      ...
      while (osd) {
        ...
        if (!strcmp(osd->predicate,"%value%"))
        {
            ...
            n += mstring_sprintf(insert,"\t\t`iri(bif:concat(\"http://%s:%s/%s/\",bif:sequence_next('<http://%s:%s/%s>')))` %s %s .\n", 
                vir_host,vir_port,field->name,vir_host,vir_port,db->name, osd->verb, pvar);
            oml_free(pvar);
        }
        else
            n += mstring_sprintf(insert,"\t\t`iri(bif:concat(\"http://%s:%s/%s/\",bif:sequence_next('<http://%s:%s/%s>')))` %s %s .\n", 
                vir_host,vir_port,field->name,vir_host,vir_port,db->name, osd->verb, osd->predicate);
        osd = osd->next;
      }
      v++;
    }
