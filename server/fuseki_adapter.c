  ...
  for (c=0; c<max; c++) {
    ...
    if (osd) {
      if (!first_concept) {
        ...
        n += mstring_sprintf(insert,"\t\t?vM MD:hasMeasurementData ?v_seq .\n", table->schema->name);
        n += mstring_sprintf(insert,"\t\t?vM MD:hasMeasurementData ?v_ts .\n", table->schema->name);
        n += mstring_sprintf(insert,"\t\t?vM MD:hasMeasurementData ?v_tc .\n", table->schema->name);
        n += mstring_sprintf(where,"\tBIND (URI(CONCAT(\"http://%s:%s/%s/\", ?struuid )) as ?vM) .\n",fus_host,fus_port,table->schema->name);
        // Build seq_no fuseki insertion
        n += mstring_sprintf(insert,"\t\t?v_seq a MD:SequenceIndex .\n");
        n += mstring_sprintf(insert,"\t\t?v_seq MD:SequenceIndexValue %s .\n", REPLACE_SEQ);
        n += mstring_sprintf(where,"\tBIND (URI(CONCAT(\"http://%s:%s/seq_no/\", ?struuid )) as ?v_seq) .\n",fus_host,fus_port);
        // Build current time (oml client) fuseki insertion
        n += mstring_sprintf(insert,"\t\t?v_tc a MD:Time .\n");
        n += mstring_sprintf(insert,"\t\t?v_tc MD:TimeValue %s .\n", REPLACE_TC);
        ...
        n += mstring_sprintf(insert,"\t\t?v_ts a MD:Time .\n");
        n += mstring_sprintf(insert,"\t\t?v_ts MD:TimeValue ?now .\n");
      ...
      n += mstring_sprintf(insert,"\t\t?vM %s ?v%d .\n", osd->verb, v);
